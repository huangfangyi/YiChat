...
import com.fanxin.app.main.utils.UrlTextViewUtils;
...
public class MomentsAdapter extends BaseAdapter {
    ...
    private UrlTextViewUtils urlTextViewUtils;
    public RelativeLayout re_edittext;
    public MomentsAdapter(MomentsActivity context, List<JSONObject> datas) {
        this.datas = datas;
        this.context = context;
        urlTextViewUtils = new UrlTextViewUtils(context);
        // 底部评论输入框
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        if (convertView == null) {
            convertView = View.inflate(context, R.layout.fx_item_moments, null);
        }
        ...
        JSONObject json = datas.get(position);
        // 如果数据出错....
...
//            });
            ...
            avatar = DemoApplication.getInstance().getUserJson().getString(FXConstant.JSON_KEY_AVATAR);
        } else {
        ...
        // 显示文章内容--加入一个对网址链接的识别处理
        urlTextViewUtils.setUrlTextView(content, holder.tv_content);
        ...
        for (int i = 0; i < goodArray.size(); i++) {
            ...
            if (json_good.getString("userID").equals(DemoHelper.getInstance().getCurrentUsernName())) {
                is_good_temp = false;
            }
        }
        // 评论
        ...
        iv_temp.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                AddPopWindow addPopWindow = new AddPopWindow(
                        (MomentsActivity) context, iv_temp,
    ...
    private void setImageViews(ViewHolder holder, String imageStr) {
        String imageBaseUrl = FXConstant.URL_AVATAR;
        ...
        if (!imageStr.equals("0")) {
            ...
            holder.image_1.setImageURI(Uri.parse(imageBaseUrl
                    + images[0]));
            holder.image_1.setOnClickListener(new ImageListener(context, images, 0));
            ...
            if (imNumb == 4) {
                ...
                holder.image_2.setImageURI(Uri
                        .parse(imageBaseUrl + images[1]));
                holder.image_2.setOnClickListener(new ImageListener(context, images,
                        1));
                holder.image_4.setVisibility(View.VISIBLE);
                holder.image_4.setImageURI(Uri
                        .parse(imageBaseUrl + images[2]));
                holder.image_4.setOnClickListener(new ImageListener(context, images,
                        2));
                holder.image_5.setVisibility(View.VISIBLE);
                holder.image_5.setImageURI(Uri
                        .parse(imageBaseUrl + images[3]));
                holder.image_5.setOnClickListener(new ImageListener(context, images,
                        3));
            } else {
                if (imNumb > 1) {
                    holder.image_2.setVisibility(View.VISIBLE);
                    holder.image_2.setImageURI(Uri
                            .parse(imageBaseUrl + images[1]));
                    holder.image_2.setOnClickListener(new ImageListener(context,
                            images, 1));
                    if (imNumb > 2) {
                        ...
                        holder.image_3.setImageURI(Uri
                                .parse(imageBaseUrl
                                        + images[2]));
                        holder.image_3
                                .setOnClickListener(new ImageListener(context,
                                        images, 2));
                        if (imNumb > 3) {
                            holder.image_4.setVisibility(View.VISIBLE);
                            holder.image_4.setImageURI(Uri
    ...
    private void setGoodTextClick(TextView mTextView2, JSONArray data,
        ...
        for (int i = 0; i < data.size(); i++) {
            ...
            if (userID_temp.equals(DemoHelper.getInstance().getCurrentUsernName())) {
                nick = DemoApplication.getInstance().getUserJson().getString(FXConstant.JSON_KEY_NICK);
    ...
    private void submitComment(String sID, String comment, TextView tv_comment,
        ...
        json.put("userID", DemoHelper.getInstance().getCurrentUsernName());
        ...
        OkHttpManager.getInstance().post(params, FXConstant.URL_SOCIAL_COMMENT, new OkHttpManager.HttpCallBack() {
            ...
            public void onResponse(JSONObject jsonObject) {
                ...
                if (code != 1000) {
                    Toast.makeText(context,"调用服务器失败...",Toast.LENGTH_SHORT).show();
                }
            }
    ...
    public void cancelGood(String sID, TextView tv_good, JSONArray jsons,
        ...
        for (int i = 0; i < jsons.size(); i++) {
            ...
            if (json.getString("userID").equals(DemoHelper.getInstance().getCurrentUsernName())) {
                jsons.remove(i);
            }
        }
        setGoodTextClick(tv_good, jsons, ll_goodmembers_temp, view, cSize);
