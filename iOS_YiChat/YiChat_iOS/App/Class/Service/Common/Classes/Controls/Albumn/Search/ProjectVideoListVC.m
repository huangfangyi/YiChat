...
static NSString *projectVideoListIdentify = @"projectVideoListIdentify";

@implementation ProjectVideoListVC
...
- (ProjectAlbumnModel *)albmunModel{
    if(!_albmunModel){
        ...
        _albmunModel.allowPickingVideo = YES;
        _albmunModel.allowPickingImage = NO;
...
- (void)collectionConfig{
    ...
    [self setProjectCollectionViewItem:^UICollectionViewCell * _Nonnull(UICollectionView * _Nonnull collection, NSIndexPath * _Nonnull index) {
        ...
        ProjectVideoCell *cell = [collection dequeueReusableCellWithReuseIdentifier:projectVideoListIdentify forIndexPath:index];
        
        cell.ProjectIsCanSelecte = ^BOOL{
            
            if(weakSelf.selecteItem.count >= weakSelf.limitPhoto){
            //    [GSYUIHelper GSYUIHelper_getAlertWithMsm:[NSString stringWithFormat:@"最多只能选择%ld个视频",weakSelf.limitPhoto]];
                return NO;
            }
            else{
                return YES;
            ...
            if(state == YES){
                ...
                [weakSelf.selecteItem enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
                    ...
                    if([num isEqualToString:identify]){
                        ...
                        [[NSNotificationCenter defaultCenter] postNotificationName:@"GSYNotificationUnselecteVideoItem" object:weakSelf.selecteItem];
                    }
                }];
            }
    ...
    [self makeCollectionView];
     ...
     [self.collectionView registerClass:[ProjectVideoCell class] forCellWithReuseIdentifier:projectVideoListIdentify];
