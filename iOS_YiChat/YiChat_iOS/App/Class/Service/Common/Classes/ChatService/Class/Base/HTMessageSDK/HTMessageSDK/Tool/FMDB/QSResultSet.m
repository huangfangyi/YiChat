...
@interface QSResultSet (Private)
- (NSMutableDictionary *)columnNameToIndexMap;
- (void)setColumnNameToIndexMap:(NSMutableDictionary *)value;
@end

@implementation QSResultSet
@synthesize query=_query;
@synthesize columnNameToIndexMap=_columnNameToIndexMap;
@synthesize statement=_statement;

+ (id)resultSetWithStatement:(QSStatement *)statement usingParentDatabase:(QSDatabase*)aDB {
    
    QSResultSet *rs = [[QSResultSet alloc] init];
    
    [rs setStatement:statement];
    [rs setParentDB:aDB];
    
    return QSDBReturnAutoreleased(rs);
}
...
- (void)dealloc {
    ...
    QSDBRelease(_columnNameToIndexMap);
...
- (NSDictionary*)resultDict {
    ...
    if (num_cols > 0) {
        ...
        return QSDBReturnAutoreleased([dict copy]);
    }
    else {
...
- (void)setParentDB:(QSDatabase *)newDb {
    _parentDB = newDb;
}
